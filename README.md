...
**v1.10.0** — Live dashboard + tuner for Bitaxe miners on your local network. Native desktop app for Mac and Windows. Click the icon, a window opens with every Bitaxe on your network — temps, hashrate, tuning recommendations, pool config. Other devices on your LAN reach the same dashboard at `http://bitaxe-baller.local`. **New in v1.10.0:** Solo block probability widget on each device's detail page — daily/monthly/yearly "1 in X" odds and a log-scaled proximity bar showing how close your best share has gotten to the network target. Auto-detects BTC, BCH, BSV, XEC (eCash), DGB (Digibyte), and NMC (Namecoin) from the pool URL; pulls live network difficulty + USD reward from free public APIs. **Pro:** remote dashboard access from outside the LAN via the bitaxeballer.com relay — outbound-only, license-key auth, one-click toggle. Free, MIT-licensed, no telemetry.
