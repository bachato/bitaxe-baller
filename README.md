...
**v1.11.0** — Live dashboard + tuner for Bitaxe miners on your local network. Native desktop app for Mac and Windows. Click the icon, a window opens with every Bitaxe on your network — temps, hashrate, tuning recommendations, pool config. Other devices on your LAN reach the same dashboard at `http://bitaxe-baller.local`. **New in v1.11.0:** Live share feed on each device's detail page — streaming accepted / rejected / new-personal-best events. Fleet best-shares leaderboard widget on the dashboard. Pro users can opt-in to the public cross-user leaderboard at [bitaxeballer.com/leaderboard](https://bitaxeballer.com/leaderboard) — four categories (all-time, weekly, per-model, lucky) so big and small miners both have a way to climb. **Pro:** remote dashboard access from outside the LAN via the bitaxeballer.com relay — outbound-only, license-key auth, one-click toggle. Free, MIT-licensed, no telemetry.
