...
- **Fleet ROI overlay** — *(2026-06-08.)* Combine block probability across all the user's devices into a single "your fleet will solve a block in ~X days at current difficulty" headline number on the dashboard. Mathematically: sum the per-device per-second probabilities, then invert. Per-chain (so a mixed BTC+BCH fleet shows two rollups). Builds on the existing `_solo_block_payload` infrastructure (now coin-aware as of 2026-06-08). ~half day, free-tier.
